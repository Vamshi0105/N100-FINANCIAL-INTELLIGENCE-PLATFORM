...
INTELLIGENCE_OUTPUT_PATH = OUTPUT_DIR / "cashflow_intelligence.xlsx"

DISTRESS_OUTPUT_PATH = OUTPUT_DIR / "distress_alerts.csv"

CAPITAL_ALLOCATION_OUTPUT_PATH = OUTPUT_DIR / "capital_allocation.csv"

DISTRIBUTION_OUTPUT_PATH = OUTPUT_DIR / "capital_allocation_distribution.csv"

PATTERN_CHANGES_OUTPUT_PATH = OUTPUT_DIR / "pattern_changes.csv"

LOG_PATH = OUTPUT_DIR / "cashflow_kpis.log"
...
def latest_two_values(
    ...
    return [float(value) for value in working_df[column].tail(2).tolist()]
...
def calculate_cfo_quality(
    ...
    if cashflow_df.empty or pnl_df.empty:
        return None, None
    ...
    merged = merged[merged["net_profit"] != 0].copy()
    ...
    return cfo_quality_score(
        merged["operating_activity"].tolist(),
...
def calculate_fcf_history(
    ...
    working_df["operating_activity"] = pd.to_numeric(
        working_df["operating_activity"],
        errors="coerce",
    )

    working_df["investing_activity"] = pd.to_numeric(
        working_df["investing_activity"],
        errors="coerce",
    )
    ...
    working_df["fcf"] = (
        working_df["operating_activity"] + working_df["investing_activity"]
    )
...
def calculate_fcf_cagr_5yr(
    ...
    history = calculate_fcf_history(cashflow_df)

    if len(history) < 5:
        return None

    values = history.tail(5).reset_index(drop=True)
    ...
    result = ((ending / beginning) ** (1 / periods) - 1) * 100
...
def calculate_fcf_conversion(
    ...
    for _, row in merged.iterrows():
        ...
        cfo = to_numeric(row["operating_activity"])

        cfi = to_numeric(row["investing_activity"])

        operating_profit = to_numeric(row["operating_profit"])

        if cfo is None or cfi is None or operating_profit is None:
            continue
...
def detect_distress(
    ...
    latest = latest_row(cashflow_df)

    if latest is None:
        return False, None, None

    cfo = to_numeric(latest.get("operating_activity"))

    cff = to_numeric(latest.get("financing_activity"))

    if cfo is None or cff is None:
        return False, cfo, cff

    return (cfo < 0 and cff > 0), cfo, cff
...
def detect_deleveraging(
    ...
    if latest_cff is None or len(borrowings) < 2:
        return False

    return latest_cff < 0 and borrowings[-1] < borrowings[-2]
...
def build_capital_allocation_history(
    ...
    working_df["financing_activity"] = pd.to_numeric(
        working_df["financing_activity"],
        errors="coerce",
    )
    ...
    if not pnl_df.empty:
        ...
        pnl_working["net_profit"] = pd.to_numeric(
            pnl_working["net_profit"],
            errors="coerce",
        )
        ...
        for _, row in pnl_working.iterrows():
            ...
            if pd.notna(year) and pd.notna(net_profit) and net_profit != 0:

                pnl_lookup[int(year)] = float(net_profit)
    ...
    for _, row in working_df.iterrows():
        ...
        cff = to_numeric(row["financing_activity"])

        cfo = 0.0 if cfo is None else cfo
        cfi = 0.0 if cfi is None else cfi
        cff = 0.0 if cff is None else cff

        year_numeric = row["year_numeric"]

        cfo_pat_ratio = None

        if pd.notna(year_numeric) and int(year_numeric) in pnl_lookup:

            net_profit = pnl_lookup[int(year_numeric)]
...
def verify_capital_allocation_coverage(
    ...
    if not missing.empty:
        ...
        missing_records = [
            (f"{row.company_id} " f"({int(row.year_numeric)})")
            for row in missing.itertuples()
        ]

        raise ValueError(
            "Missing capital allocation records: " + ", ".join(missing_records)
        )

    logging.info(
        "Capital allocation coverage passed " "for %s company-year records",
        len(expected),
    )
...
def generate_distribution_summary(
    ...
    latest_rows = (
        capital_allocation_df.sort_values(
            by=[
                "company_id",
                "year_numeric",
            ],
...
def generate_pattern_changes(
    ...
    working_df = capital_allocation_df.sort_values(
        by=[
            "company_id",
            "year_numeric",
        ],
    ...
    for company_id, group in working_df.groupby("company_id"):
        ...
        for index in range(
            ...
            previous_pattern = previous_row["pattern_label"]

            current_pattern = current_row["pattern_label"]
                ...
                records.append(
                    {
                        ...
                        "change_description": (
                            f"{previous_pattern} " f"-> " f"{current_pattern}"
                        ),
                    }
                )
...
def get_sector(
    ...
    if pd.notna(broad_sector) and str(broad_sector).strip():
        return str(broad_sector).strip()

    if pd.notna(sub_sector) and str(sub_sector).strip():
        return str(sub_sector).strip()
...
def generate_cashflow_intelligence(
    ...
    for company_id in company_ids:
        ...
        company_sector = sectors_df[sectors_df["company_id"] == company_id].copy()

        sector = get_sector(company_sector)
        ...
        fcf_cagr = calculate_fcf_cagr_5yr(company_cashflow)
        ...
        distress_flag, latest_cfo, latest_cff = detect_distress(company_cashflow)
        ...
        company_capital_history = build_capital_allocation_history(
            company_cashflow,
            company_pnl,
        )

        if not company_capital_history.empty:

            capital_history_frames.append(company_capital_history)

            latest_capital_row = company_capital_history.sort_values(
                by="year_numeric",
                ascending=False,
            ).iloc[0]

            capital_allocation_label = latest_capital_row["pattern_label"]
    ...
    intelligence_df = intelligence_df.sort_values(by="company_id").reset_index(
        drop=True
    )

    distress_df = distress_df.sort_values(by="company_id").reset_index(drop=True)

    capital_allocation_df = capital_allocation_df.sort_values(
        by=[
            "company_id",
            "year_numeric",
        ]
...
def verify_company_coverage(
    ...
    generated_companies = set(intelligence_df["company_id"].dropna().tolist())

    missing = expected_companies - generated_companies

    if missing:

        raise ValueError(
            "Companies missing cash flow intelligence: " + ", ".join(sorted(missing))
        )
...
def run_cashflow_intelligence() -> None:
    ...
    logging.info("Starting Cash Flow Intelligence " "and Capital Allocation Module")
    ...
    (
        ...
        distress_df,
        capital_allocation_df,
    ) = generate_cashflow_intelligence(data)
    ...
    distribution_df = generate_distribution_summary(capital_allocation_df)

    pattern_changes_df = generate_pattern_changes(capital_allocation_df)
    ...
    print(f"Capital allocation records: " f"{len(capital_allocation_df)}")

    print(f"Distress alerts: " f"{len(distress_df)}")

    print(f"Pattern changes: " f"{len(pattern_changes_df)}")

    print("\nOutputs:")

    print(f"  {INTELLIGENCE_OUTPUT_PATH}")

    print(f"  {DISTRESS_OUTPUT_PATH}")

    print(f"  {CAPITAL_ALLOCATION_OUTPUT_PATH}")

    print(f"  {DISTRIBUTION_OUTPUT_PATH}")

    print(f"  {PATTERN_CHANGES_OUTPUT_PATH}")
