...
FCF_PATH = OUTPUT_DIR / "cashflow_intelligence.xlsx"
...
def load_financial_data() -> pd.DataFrame:
    ...
    if df.empty:
        ...
        raise ValueError("financial_ratios returned no rows")
    ...
    df["year_numeric"] = pd.to_numeric(
        df["year_text"].str.extract(r"(\d{4})")[0],
        errors="coerce",
    )
    ...
    annual_df = df[df["year_text"].str.endswith("-03")].copy()
...
def load_sector_data() -> pd.DataFrame:
    ...
    if df.empty:
        ...
        raise ValueError("sectors table returned no rows")

    df["company_id"] = df["company_id"].astype(str).str.strip().str.upper()

    df["broad_sector"] = df["broad_sector"].astype(str).str.strip()
...
def load_fcf_data() -> pd.DataFrame:
    ...
    if not FCF_PATH.exists():
        ...
        raise FileNotFoundError(
            "Cash-flow intelligence output not found: " f"{FCF_PATH}"
        )

    df = pd.read_excel(FCF_PATH)
...
def impute_sector_medians(
    ...
    logger.info("Missing values BEFORE sector imputation:")

    before = result[FEATURES].isna().sum()
    ...
    after = result[FEATURES].isna().sum()

    logger.info("Missing values AFTER sector imputation:")
    ...
    logger.info("Sector-median imputation PASSED")
...
def assign_archetype_names(
    ...
    ranks = centroid_df.rank(pct=True)
    ...
    distressed_cluster = int(distressed_score.idxmax())

    labels = {distressed_cluster: "Distressed"}

    remaining = [
        cluster_id for cluster_id in range(N_CLUSTERS) if cluster_id not in labels
    ]
    ...
    quality_cluster = max(
        remaining,
        key=lambda cluster_id: quality_score.iloc[cluster_id],
    )

    labels[quality_cluster] = "High-Quality Growth"
    ...
    emerging_cluster = max(
        remaining,
        key=lambda cluster_id: emerging_growth_score.iloc[cluster_id],
    )

    labels[emerging_cluster] = "Emerging Growth"

    remaining.remove(emerging_cluster)
    ...
    defensive_cluster = max(
        remaining,
        key=lambda cluster_id: defensive_score.iloc[cluster_id],
    )

    labels[defensive_cluster] = "Defensive Dividend"
    ...
    if len(remaining) != 1:
        ...
        raise RuntimeError("Unable to assign unique archetype labels")

    labels[remaining[0]] = "Value Cyclicals"
...
def run_clustering(
    ...
    cluster_names = assign_archetype_names(
        model,
        scaler,
    )
    ...
    result["cluster_name"] = [
        cluster_names[int(cluster_id)] for cluster_id in cluster_ids
    ]

    result["distance_from_centroid"] = pd.Series(assigned_distances).round(6)
...
def validate_clusters(
    ...
    logger.info("Validating cluster output...")
    ...
    if len(result) != EXPECTED_COMPANIES:
        ...
        raise AssertionError(
            f"Expected {EXPECTED_COMPANIES} rows, " f"found {len(result)}"
        )
    ...
    if missing_columns:
        ...
        raise AssertionError("Missing output columns: " + ", ".join(missing_columns))
    ...
    if result["cluster_id"].isna().any():
        ...
        raise AssertionError("Null cluster_id detected")
    ...
    if (
        ...
        raise AssertionError("Cluster IDs must be between 0 and 4")

    # --------------------------------------------------------
    # Every cluster should actually exist.
    # --------------------------------------------------------

    cluster_count = result["cluster_id"].nunique()

    if cluster_count != N_CLUSTERS:

        raise AssertionError(
            f"Expected {N_CLUSTERS} clusters, " f"found {cluster_count}"
        )
    ...
    actual_names = set(result["cluster_name"].dropna().unique())
    ...
    if result["distance_from_centroid"].isna().any():

        raise AssertionError("Null distance_from_centroid detected")

    if (result["distance_from_centroid"] < 0).any():

        raise AssertionError("Negative centroid distance detected")
...
def main() -> None:
    ...
    logger.info("Starting Day 36 KMeans Clustering")

    # --------------------------------------------------------
    # 1. Prepare data
    # --------------------------------------------------------

    df = prepare_clustering_data()
    ...
    logger.info("Day 36 KMeans Clustering COMPLETE")
