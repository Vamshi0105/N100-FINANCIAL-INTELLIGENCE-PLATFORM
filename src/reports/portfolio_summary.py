...
PORTFOLIO_OUTPUT_PATH = PORTFOLIO_DIR / "portfolio_summary.pdf"

CASHFLOW_INTELLIGENCE_PATH = OUTPUT_DIR / "cashflow_intelligence.xlsx"

LOG_PATH = OUTPUT_DIR / "portfolio_summary.log"
...
def load_cashflow_intelligence() -> pd.DataFrame:
    ...
    df = pd.read_excel(CASHFLOW_INTELLIGENCE_PATH)
...
def get_company_sector(
    ...
    if (
        ...
        and pd.notna(broad_sector)
        and str(broad_sector).strip()
    ):
    ...
    if sub_sector is not None and pd.notna(sub_sector) and str(sub_sector).strip():
...
def get_trend_arrow(
    ...
    latest = to_numeric(latest_value)

    previous = to_numeric(previous_value)
...
def build_kpi_table(
    ...
    latest_pnl, previous_pnl = get_latest_two_rows(company_pnl)

    latest_ratios, previous_ratios = get_latest_two_rows(company_ratios)

    latest_intelligence = None

    if not company_intelligence.empty:

        latest_intelligence = company_intelligence.iloc[0]

    kpis = []

    # Revenue

    latest_revenue = latest_pnl.get("sales") if latest_pnl is not None else None

    previous_revenue = previous_pnl.get("sales") if previous_pnl is not None else None
    ...
    latest_profit = latest_pnl.get("net_profit") if latest_pnl is not None else None

    previous_profit = (
        previous_pnl.get("net_profit") if previous_pnl is not None else None
    )
    ...
    previous_roe = (
        previous_ratios.get("return_on_equity_pct")
    ...
    previous_roce = (
        previous_ratios.get("return_on_capital_employed_pct")
        if previous_ratios is not None
        else None
    )
    ...
    previous_de = (
        previous_ratios.get("debt_to_equity") if previous_ratios is not None else None
    )
...
def generate_portfolio_summary() -> Path:
    ...
    logging.info("Starting Day 35 Portfolio Summary PDF generation")
    ...
    intelligence_df = load_cashflow_intelligence()

    company_ids = sorted(data["companies"]["company_id"].dropna().unique().tolist())
    ...
    print("\nDay 35 Portfolio Summary " "completed successfully.")

    print(f"Companies included: " f"{len(company_ids)}")

    print("\nPortfolio PDF:")

    print(f"  {PORTFOLIO_OUTPUT_PATH}")
