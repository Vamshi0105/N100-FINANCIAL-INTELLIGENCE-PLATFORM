...
BRANDING_IMAGE_PATH = REPORT_ASSET_DIR / "n100_branding_texture.png"
...
LOG_PATH = OUTPUT_DIR / "tearsheet.log"
...
def load_output_data() -> dict[
    ...
    if PROS_CONS_PATH.exists():
        ...
        pros_cons_df["company_id"] = pros_cons_df["company_id"].apply(
            normalize_company_id
        )

        data["pros_cons"] = pros_cons_df
        ...
        logging.warning("Pros and cons file not found")

    if CASHFLOW_INTELLIGENCE_PATH.exists():

        intelligence_df = pd.read_excel(CASHFLOW_INTELLIGENCE_PATH)

        intelligence_df["company_id"] = intelligence_df["company_id"].apply(
            normalize_company_id
        )

        data["cashflow_intelligence"] = intelligence_df
        ...
        data["cashflow_intelligence"] = pd.DataFrame()

        logging.warning("Cash flow intelligence file not found")
...
def get_company_name(
    ...
    value = company.iloc[0].get("company_name")
...
def get_company_sector(
    ...
    sector = sectors_df[sectors_df["company_id"] == company_id]
    ...
    value = row.get("broad_sector")

    if value is not None and pd.notna(value) and str(value).strip():

        return str(value).strip()

    value = row.get("sub_sector")
...
def latest_row(
    ...
    if "year_numeric" in working_df.columns:
...
def has_minimum_data(
    ...
    company_pnl = pnl_df[pnl_df["company_id"] == company_id]
    ...
    year_count = int(company_pnl["year_numeric"].dropna().nunique())
...
def build_kpi_section(
    ...
    sales = latest_pnl.get("sales") if latest_pnl is not None else None

    net_profit = latest_pnl.get("net_profit") if latest_pnl is not None else None

    roe = (
        latest_ratios.get("return_on_equity_pct") if latest_ratios is not None else None
    )
    ...
    debt_equity = (
        latest_ratios.get("debt_to_equity") if latest_ratios is not None else None
    )

    cfo_quality = (
        latest_intelligence.get("cfo_quality_score")
...
def get_company_history(
    ...
    company_df = company_df.tail(years)

    available_columns = ["year_numeric"] + [
        column for column in columns if column in company_df.columns
    ]

    return company_df[available_columns]
...
def build_bar_chart(
    ...
    for _, row in history_df.iterrows():
        ...
        value = to_numeric(row.get(value_column))

        values.append(value if value is not None else 0)

        year = row.get("year_numeric")

        labels.append(str(year) if pd.notna(year) else "")
...
def build_ratio_chart(
    ...
    for _, row in history_df.iterrows():
        ...
        roe = to_numeric(row.get("return_on_equity_pct"))

        roce = to_numeric(row.get("return_on_capital_employed_pct"))

        roe_values.append(roe if roe is not None else 0)

        roce_values.append(roce if roce is not None else 0)

        labels.append(str(row.get("year_numeric")))
...
def build_balance_sheet_chart(
    ...
    for _, row in history_df.iterrows():
        ...
        equity_capital = to_numeric(row.get("equity_capital")) or 0

        reserves = to_numeric(row.get("reserves")) or 0

        borrowings = to_numeric(row.get("borrowings")) or 0

        other_liabilities = to_numeric(row.get("other_liabilities")) or 0

        equity_values.append(equity_capital + reserves)

        borrowing_values.append(borrowings)

        liability_values.append(other_liabilities)
...
def build_cashflow_chart(
    ...
    for column in columns:
        ...
        value = to_numeric(latest_cashflow.get(column))
...
def build_page_one(
    ...
    ratios_df = data["financial_ratios"]

    sectors_df = data["sectors"]

    intelligence_df = output_data["cashflow_intelligence"]
    ...
    latest_pnl = latest_row(company_pnl)

    latest_ratios = latest_row(company_ratios)

    company_intelligence = intelligence_df[intelligence_df["company_id"] == company_id]

    latest_intelligence = (
        company_intelligence.iloc[0] if not company_intelligence.empty else None
    )
    ...
    story.append(build_branding_strip())
...
def build_page_two(
    ...
    balancesheet_df = data["balancesheet"]

    cashflow_df = data["cashflow"]

    pros_cons_df = output_data["pros_cons"]
    ...
    company_cashflow = cashflow_df[cashflow_df["company_id"] == company_id].copy()

    latest_cashflow = latest_row(company_cashflow)

    company_pros_cons = pros_cons_df[pros_cons_df["company_id"] == company_id].copy()

    company_intelligence = intelligence_df[
        intelligence_df["company_id"] == company_id
    ].copy()
    ...
    pros = company_pros_cons[company_pros_cons["type"].astype(str).str.lower() == "pro"]

    cons = company_pros_cons[company_pros_cons["type"].astype(str).str.lower() == "con"]
    ...
    if not company_intelligence.empty:
        ...
        value = company_intelligence.iloc[0].get("capital_allocation_label")

        if value is not None and pd.notna(value):

            capital_allocation = str(value).strip()
...
def generate_tearsheet(
    ...
    output_path = output_directory / f"{company_id}_tearsheet.pdf"
...
def generate_batch_tearsheets() -> tuple[
    ...
    logging.info("Starting Day 34 batch tearsheet generation")
    ...
    for company_id in company_ids:
        ...
        valid_data, year_count = has_minimum_data(
            company_id,
            data,
            minimum_years=3,
        )
...
def get_latest_company_metrics(
    ...
    company_ratios = ratios_df[ratios_df["company_id"] == company_id]
    ...
    return {
        "company_id": company_id,
        "Revenue": (latest_pnl.get("sales") if latest_pnl is not None else None),
        "Net Profit": (
            latest_pnl.get("net_profit") if latest_pnl is not None else None
        ),
        "ROE": (
...
def build_sector_report(
    ...
    filename = f"{safe_filename(sector_name)}" "_report.pdf"

    output_path = SECTOR_REPORT_DIR / filename
    ...
    for company_id in sector_company_ids:
        ...
        record["Company"] = get_company_name(
            company_id,
            companies_df,
        )
    ...
    for column in median_columns:
        ...
        if column in [
            ...
            formatted = format_percent(median_value)

        else:

            formatted = format_number(median_value)
    ...
    for _, row in metrics_df.sort_values("Company").iterrows():
        ...
        table_data.append(
            [
                ...
                Paragraph(
                    format_percent(row.get("ROCE")),
                    styles["small"],
                ),
                Paragraph(
                    format_number(row.get("Debt / Equity")),
                    styles["small"],
                ),
                Paragraph(
                    format_percent(row.get("Net Profit Margin")),
                    styles["small"],
                ),
                Paragraph(
                    format_number(
                ...
                Paragraph(
                    format_number(row.get("CFO Quality")),
                    styles["small"],
                ),
            ]
        )
...
def generate_sector_reports() -> list[Path]:

    logging.info("Starting batch sector report generation")
    ...
    valid_sectors = sectors_df[sectors_df["broad_sector"].notna()].copy()

    valid_sectors["broad_sector"] = (
        valid_sectors["broad_sector"].astype(str).str.strip()
    )

    valid_sectors = valid_sectors[valid_sectors["broad_sector"] != ""]

    generated_reports: list[Path] = []

    for sector_name, group in valid_sectors.groupby("broad_sector"):

        company_ids = sorted(group["company_id"].dropna().unique().tolist())
...
def run_day_34() -> None:
    ...
    logging.info("Starting Day 34 Batch Report Generation")

    generated_files, skipped_records = generate_batch_tearsheets()

    sector_reports = generate_sector_reports()

    print("\nDay 34 Batch Report Generation " "completed successfully.")

    print(f"\nCompany tearsheets generated: " f"{len(generated_files)}")

    print(f"Companies skipped: " f"{len(skipped_records)}")

    print(f"Sector reports generated: " f"{len(sector_reports)}")

    print("\nTearsheet directory:")

    print(f"  {TEARSHEET_DIR}")

    print("\nSector report directory:")

    print(f"  {SECTOR_REPORT_DIR}")

    print("\nSkipped tearsheets:")

    print(f"  {SKIPPED_TEARSHEETS_PATH}")
